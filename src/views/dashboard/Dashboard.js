...
const Dashboard = ({ isSidebarOpen }) => {
  ...
  const filteredAndSortedCards = useMemo(() => {
    let result = cardData.filter(
      (card) =>
        card.label.toLowerCase().includes(searchTerm.toLowerCase()) &&
        (selectedCategories.length === 0 ||
          selectedCategories.includes(card.category)) &&
        Number(insuranceDetails[card.id].monthlyPremium.replace("$", "")) <=
          priceRange
    );
        ...
        >
          {rowCards.map((card) => (
            ...
            >
              ...
              >
                {insuranceDetails[card.id].monthlyPremium}/month
              </div>
            </div>
          ))}
        </div>
        {selectedCard &&
          rowCards.some((card) => card.id === selectedCard.id) && (
            <div
              style={{
                margin: "0 20px 20px",
                padding: "20px",
            ...
            >
              ...
              >
                <h2 style={{ fontSize: "24px", color: "#333" }}>
                  {insuranceDetails[selectedCard.id].title}
                </h2>
              </div>
              <p
                style={{
              ...
              >
                {insuranceDetails[selectedCard.id].description}
              </p>
              <h3
                style={{
                  fontSize: "18px",
              ...
              <ul style={{ paddingLeft: "20px", marginBottom: "15px" }}>
                {insuranceDetails[selectedCard.id].keyFeatures.map(
                  (feature, index) => (
                    <li
                      key={index}
                      style={{
              ...
              <p
                style={{ fontSize: "16px", color: "#333", marginBottom: "5px" }}
              >
                <strong>Coverage Amount:</strong>{" "}
                {insuranceDetails[selectedCard.id].coverageAmount}
              </p>
              <p style={{ fontSize: "16px", color: "#333" }}>
                <strong>Monthly Premium:</strong>{" "}
                {insuranceDetails[selectedCard.id].monthlyPremium}
              </p>
            </div>
          )}
      </React.Fragment>
  ...
  const categories = useMemo(
    () => [...new Set(cardData.map((card) => card.category))],
    []
  );
  ...
  return (
    <div
      ...
      style={{ display: "flex", height: "100vh", marginBottom: "40px" }}
    >
      <div
        className="dashboard-content"
        style={{ flexGrow: 1, padding: "20px", marginBottom: "40px" }}
      >
        <div
          style={{
        ...
        >
          ...
          >
            <h1
              style={{ fontSize: "28px", color: "#2c3e50", fontWeight: "bold" }}
            >
              Welcome back, User!
            </h1>
          </div>
